# Product goal — chat-first trip OS

> **Status:** Product behavior SSOT for **agentic-trip** (not Wandr).  
> **OpenSpec root:** this repo — `openspec/` (main specs seeded from archived `chat-first-trip-os-blueprint`).  
> **Formerly:** `chat-first-trip-os.md` (renamed for clarity).  
...
7. **Lean agent checkpoint later** (`context.md` style per package). Until then, this file is the fat product bible.
...
This product’s HTTP is **not** Wandr OpenAPI. Do not reuse or invent `guideagent` routes here.
...
Wandr Explore today has **no** nearby HTTP. This product **does** need a real nearby query (P7) — that is not Wandr `places?destination_id=`. Do not pretend Wandr’s catalog feed is this contract.
...
Harness cases this repo MUST include:
...
| Region multi-hub (“Meghalaya, trek, multi-day”) | Region / multi-hub trek shape; not a country-centroid scrape |
...
Delivery packages live under `system-docs/phase-slices/`. Implement one OpenSpec phase change at a time (`p0-foundation` → … → `p9-hardening`), sub-phase by sub-phase.

| Phase | Ships | Proof intent |
|-------|--------|----------------|
| **P0** Foundation | Repo layout `/src`, obs/evals skeleton, health | Health endpoint + empty shell |
...
- Wandr `guideagent` blueprint / AGENT / lean `context.md` patterns — principles, failure tables, gateways, bounds
...
Wandr remains the shipped city-hub planner. **agentic-trip** is the sibling chat-first trip OS in this repo.
